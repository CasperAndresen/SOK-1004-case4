...
*En god verdi er en verdi som gjør det enkelt å lese og tolke dataene samtidig som du ikke mister noe betydningsfullt.*
...
df_ny %>%
  ggplot(aes(verdi)) +
  geom_histogram(aes(fill = statistikkvariabel),binwidth=500) +
  facet_grid(~næring)
```
